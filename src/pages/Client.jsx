...
function Client() {
	...
	const [isActiveTabClient, setActiveTabsClient] = useState([true, false, false, false]);
	...
	return (
		<main>
					...
					<div id="button-group" className="flex flex-row gap-2 justify-center">
						<button
							...
							className={`px-6 py-1 text-xl rounded-md ${
								isActiveTabClient[0] ? 'bg-[#0C3764] text-white' : 'bg-gray-200 text-black'
							}`}>
							TIPO A
						</button>
						...
						<button
							...
							className={`px-6 py-1 text-xl rounded-md ${
								isActiveTabClient[1] ? 'bg-[#0C3764] text-white' : 'bg-gray-200 text-black'
							}`}>
							ULTIMAS COMPRAS
						</button>
						...
						<button
							...
							className={`px-6 py-1 text-xl rounded-md ${
								isActiveTabClient[2] ? 'bg-[#0C3764] text-white' : 'bg-gray-200 text-black'
							}`}>
							ITEM MAS COMPRADOS
						</button>
						...
						<button
							...
							className={`px-6 py-1 text-xl rounded-md ${
								isActiveTabClient[3] ? 'bg-[#0C3764] text-white' : 'bg-gray-200 text-black'
							}`}>
							CREDITOS Y COBRANZAS
						</button>
					</div>
					<div id="content">
						<div id="content-0" className={`${isActiveTabClient[0] ? 'flex flex-row' : 'hidden'}`}>
							...
							<div id="historial" className="w-1/2">
								{
                  selectClient && <DataTypeClient data={dataHistorial} numDocumento={selectClient.numDocumento}/>
                }
							</div>
						</div>
						...
						<div id="content-2" className={`${isActiveTabClient[2] ? 'flex' : 'hidden'}`}>
							Texto 3
						</div>
						<div id="content-3" className={`${isActiveTabClient[3] ? 'flex' : 'hidden'}`}>
							Texto 4
						</div>
					</div>
				</div>
			...
			<div className="flex flex-row items-center gap-4 p-4 bg-[#0C3764] text-white">
				<button onClick={() => handleTabs(1)} className="text-3xl font-medium">
					{' '}
					ITEM{' '}
				</button>
				...
				<div className="flex justify-end items-center gap-2">
					...
					<div className="flex gap-2" onClick={() => handleTabs(1)}>
						<input type="text" placeholder="DFG" className="py-1 px-2 rounded-md focus:outline-none text-black " />
			...
			<div className={`${isActiveTabs[1] ? 'flex' : 'hidden'}`}>
				<div id="tabs" className="flex flex-col gap-4 p-4">
					<div id="button-group" className="flex flex-col gap-2 justify-center">
						<button
							onClick={() => handleTabsItem(0)}
							className={`px-6 py-1 text-xl rounded-md ${
								isActiveTabItem[0] ? 'bg-[#0C3764] text-white' : 'bg-gray-200 text-black'
							}`}>
							TD
						</button>
						<button
							...
							className={`px-6 py-1 text-xl rounded-md ${
								isActiveTabItem[1] ? 'bg-[#0C3764] text-white' : 'bg-gray-200 text-black'
							}`}>
							S.I
						</button>
						<button
							...
							className={`px-6 py-1 text-xl rounded-md ${
								isActiveTabItem[2] ? 'bg-[#0C3764] text-white' : 'bg-gray-200 text-black'
							}`}>
							PH
						</button>
					</div>
					<div id="content">
						<div id="content-4" className={`${isActiveTabItem[0] ? 'flex' : 'hidden'}`}>
							Texto 5
						</div>
						<div id="content-5" className={`${isActiveTabItem[1] ? 'flex' : 'hidden'}`}>
							Texto 6
						</div>
						<div id="content-6" className={`${isActiveTabItem[2] ? 'flex' : 'hidden'}`}>
							Texto 7{' '}
						</div>
					</div>
				</div>
